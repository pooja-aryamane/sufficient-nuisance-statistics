...
def create_data_chexpert(new_dir, chexpert_dir, K):
    t_chexpert_df = pd.read_csv('/CheXpert-v1.0/train.csv')
    v_chexpert_df = pd.read_csv('/CheXpert-v1.0/valid.csv')
    chexpert_df = pd.concat([t_chexpert_df,v_chexpert_df])
    print(chexpert_df.shape)
    ctrain_data, cval_data, ctest_data = train_test_split_stratified(chexpert_df,0.8,0.2,0)
    ...
    full_data = pd.concat([ctrain_data, cval_data])
    os.makedirs(new_dir, exist_ok=True)
    os.makedirs(os.path.join(new_dir, "imgs/"), exist_ok=True)
    ...
    for i,row in full_data.iterrows():
        ...
        if i%10==0:
            print(f'{round(i/full_data.shape[0],2)}% done')
    
    full_data.to_csv(os.path.join(new_dir,'full_data_chexpert_newdata.csv'))
...
def resetchex(chexpertdir):
    chexdf=pd.read_csv(chexpertdir)
    for i,row in chexdf.iterrows():
        img_path = '/'+row['old_path']
...
if __name__=="__main__":
    ...
    resetchex(chexpert_dir)
    print("chexpert data reset!")
