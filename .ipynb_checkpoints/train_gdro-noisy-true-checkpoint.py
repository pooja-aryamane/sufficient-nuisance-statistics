...
def start_training_robust(args,model, train_dataloader, val_dataloader, device):
   ...
   # scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.95)
    ...
    for epoch in range(args.MAX_EPOCHS): 
        ...
        experiment_name = f'noisytrue_group{group_name}_epoch_{epoch+1}+weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}+stepsize_{args.STEP_SIZE}'

        experiment_name_no_epoch = f'noisytrue_group{group_name}+weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}+stepsize_{args.STEP_SIZE}'
...
if __name__=="__main__":
    ...
    experiment = f'noisytrue_exp+groups_{args.n_groups}_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}_transformation_{args.TRANSFORM}+stepsize_{args.STEP_SIZE}'
    ...
    chexpert_dir = '/scratch/paa9751/mlhc-project/resized_data/chexpert/full_data_chexpert_noisy_true.csv'
    mimic_dir = '/scratch/paa9751/mlhc-project/resized_data/mimic/full_data_mimic_noisy_true.csv'

    # if args.GROUP_CNAME == 'true_group_idx':
    #     chexpert_dir = '/scratch/paa9751/mlhc-project/resized_data/chexpert/full_data_chexpert_full.csv'
    #     mimic_dir = '/scratch/paa9751/mlhc-project/resized_data/mimic/full_data_mimic_full.csv'
    # else:
    #     chexpert_dir = '/scratch/paa9751/mlhc-project/resized_data/chexpert/full_data_chexpert_groupremoved.csv' #removing low accuracy subgroup
    #     mimic_dir = '/scratch/paa9751/mlhc-project/resized_data/mimic/full_data_mimic_groupremoved.csv'
