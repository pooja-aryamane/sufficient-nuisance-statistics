...
def start_training_robust(args,model, train_dataloader, val_dataloader, device):
   ...
   # scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.95)
    ...
    for epoch in range(args.MAX_EPOCHS): 
        ...
        experiment_name = f'ysnsnew_group{group_name}_epoch_{epoch+1}+weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'

        experiment_name_no_epoch = f'ysnsnew_group{group_name}+weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'
...
if __name__=="__main__":
    ...
    experiment = f'ysnsnew_exp+groups_{args.n_groups}_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}_transformation_{args.TRANSFORM}'
