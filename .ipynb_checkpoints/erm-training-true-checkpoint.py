...
def start_training(args, model, train_dataloader, val_dataloader, device):
   ...
   # scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.95)
    ...
    for epoch in range(args.MAX_EPOCHS): 
        ...
        experiment_name = f'truenew_epoch_{epoch+1}+weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'

        experiment_name_no_epoch = f'truenew_weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'
...
if __name__=="__main__":
    ...
    experiment = f'truenew_wd_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'
    ...
    chexpert_dir = '/scratch/paa9751/mlhc-project/resized_data/chexpert/full_data_chexpert_true_group_balanced.csv'
    mimic_dir = '/scratch/paa9751/mlhc-project/resized_data/mimic/full_data_mimic_true_group_balanced.csv'
    
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)
  
    train_dataloader, val_dataloader, test_dataloader = load_resized_data(chexpert_dir, mimic_dir, args.IMAGE_SIZE, args.NORMALISE, args.TRANSFORM, args.NWORKERS, args.BATCH_SIZE, args.BORDER_SZ, 'true_group_idx') 
