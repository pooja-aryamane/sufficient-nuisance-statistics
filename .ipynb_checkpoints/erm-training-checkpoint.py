...
def start_training(args, model, train_dataloader, val_dataloader, device):
   ...
   # scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.95)
    ...
    for epoch in range(args.MAX_EPOCHS): 
        ...
        train_losses_by_group = {i: [] for i in range(8)}
        train_loss_by_group = {i: 0 for i in range(8)}

        train_preds_by_group = {i: [] for i in range(8)}
        train_target_by_group = {i: [] for i in range(8)}
        train_auroc_by_group = {i: [] for i in range(8)}
        ...
        val_losses_by_group = {i: [] for i in range(8)} #ngroups is always 4 - this is only across true groups 
        val_loss_by_group = {i: 0 for i in range(8)}
        val_auroc_by_group = {i: [] for i in range(8)}
        val_f1_by_group = {i: [] for i in range(8)}
        
        preds_by_group = {i: [] for i in range(8)}
        target_by_group = {i: [] for i in range(8)}
        ...
        experiment_name = '0.4-0.6'+f'{args.NGROUPS}+new_epoch_{epoch+1}+weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'

        experiment_name_no_epoch = '0.4-0.6'+f'{args.NGROUPS}+new_weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'
        ...
        if epoch%10==0:
            ...
            torch.save(model, f'{args.OUTDIR}/models/{experiment_name}_model_weights.pth')
    ...
    return model_minloss_path, model_maxacc_path
    
if __name__=="__main__":
    parser = argparse.ArgumentParser()
    # Project name 
    parser.add_argument("--PROJECT_NAME", type=str, default = 'balanced-erm', help="project_name")
    ...
    parser.add_argument("--NGROUPS", type=int, default=2, help = "ngroups to balance")
    ...
    experiment = '0.4-0.6'+f'{args.NGROUPS}+new_wd_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'
    ...
    if args.NGROUPS==2:
        chexpert_dir = '/scratch/paa9751/mlhc-project/resized_data/chexpert/full_data_chexpert_y2group_balanced.csv'
        mimic_dir = '/scratch/paa9751/mlhc-project/resized_data/mimic/full_data_mimic_y2group_balanced.csv'
    elif args.NGROUPS==4:
        chexpert_dir = '/scratch/paa9751/mlhc-project/resized_data/chexpert/full_data_chexpert_y4group_balanced_0.4_0.6.csv'
        mimic_dir = '/scratch/paa9751/mlhc-project/resized_data/mimic/full_data_mimic_y4group_balanced_0.4_0.6.csv'
    elif args.NGROUPS==8:
        chexpert_dir = '/scratch/paa9751/mlhc-project/resized_data/chexpert/full_data_chexpert_y8group_balanced.csv'
        mimic_dir = '/scratch/paa9751/mlhc-project/resized_data/mimic/full_data_mimic_y8group_balanced.csv'
    
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)
  
    train_dataloader, val_dataloader, test_dataloader = load_resized_data(chexpert_dir, mimic_dir, args.IMAGE_SIZE, args.NORMALISE, args.TRANSFORM, args.NWORKERS, args.BATCH_SIZE, args.BORDER_SZ, 'new_sns_4') 
    ...
    model_minloss_path, model_maxacc_path = start_training(args, model, train_dataloader, val_dataloader, device)
    ...
    df_minloss.to_csv(f'{args.OUTDIR}/fullpreds/{experiment}_minloss_preds.csv')
    df_maxacc.to_csv(f'{args.OUTDIR}/fullpreds/{experiment}_maxacc_preds.csv')
