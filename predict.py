...
if __name__=="__main__":
    ...
    acc_modelpath='stage2-outdir/seed_20230514+grouptrue_group_idx+stepsize_2+weightdecay_1.0+pretrained_True+batchsize_256+lr_0.0001+transform_True_max_val_acc.pth'
    #loss_modelpath='balanced-erm/weightdecay_0.01+pretrained_True+batchsize_256+lr_0.001+transform_True_lowest_val_loss.pth'

    print('getting preds..')
    df_pred_acc = get_full_data_predictions(acc_modelpath,device)
    #df_pred_loss = get_full_data_predictions(loss_modelpath,device)
    print('pred done, saving combined df')
    df_pred_acc.to_csv('true_group_dro_predictions.csv', index=False)
    #df_pred_loss.to_csv('balanced-erm/full_data_predictions_loss.csv', index=False)
