...
def start_training(args, model, train_dataloader, val_dataloader, device):
   ...
   # scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.95)
    ...
    for epoch in range(args.MAX_EPOCHS): 
        ...
        experiment_name = f'epoch_{epoch+1}+weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}+split_{args.SPLIT_IDX}'

        experiment_name_no_epoch = f'weightdecay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}+split_{args.SPLIT_IDX}'
...
if __name__=="__main__":
    ...
    experiment = f'split_{args.SPLIT_IDX}+decay_{str(args.WEIGHT_DECAY)}+pretrained_{str(args.PRETRAINED)}+batchsize_{str(args.BATCH_SIZE)}+lr_{args.LEARNING_RATE}+transform_{args.TRANSFORM}'
    ...
    chexpert_dir = '/scratch/paa9751/mlhc-project/resized_data/chexpert/full_data_chexpert_full_new.csv'
    ...
    # test_path='stage1-outdir/weightdecay_0.001+pretrained_False+batchsize_64+lr_1e-05+transform_True_max_val_acc.pth'

    predictions = get_predictions(model_maxacc_path, model_minloss_path, args.BORDER_SZ, 1-args.SPLIT_IDX, device)

    # predictions = get_predictions(test_path, test_path, args.BORDER_SZ, 1-args.SPLIT_IDX, device)
    
    predictions.to_csv(f'{args.OUTDIR}/{experiment}_opposite_split_predictions.csv')
